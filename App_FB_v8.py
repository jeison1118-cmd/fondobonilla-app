...
# -*- coding: utf-8 -*-
# App_FB_v7.py — Fondo Bonilla (Excel + UI) con plan_inicio, llaves únicas y validaciones robustas
# Autor: M365 Copilot para JEISON MAURICIO BONILLA
...
def inject_css():
    st.markdown(
        f"""
        <style>
          .stMetric > div > div:nth-child(2) {{ color: {PRIMARY_COLOR}; }}
          .stButton>button {{ background:{PRIMARY_COLOR}; color:white; border-radius:8px; }}
          .css-1d391kg {{ background: {BG_GRADIENT}; }}
        </style>
        """,
...
def init_empty_frames():
    clientes = pd.DataFrame(columns=[
        "cliente_id","nombre","identificacion","telefono","email","creado_en","actualizado_en"
    ])
    prestamos = pd.DataFrame(columns=[
        "prestamo_id","display_id","cliente_id","monto","tasa_mensual","fecha_inicio","plan_inicio","plazo_meses",
        "frecuencia","saldo_capital","estado","ultimo_calculo","cuota_fija","creado_en","actualizado_en"
    ])
    pagos = pd.DataFrame(columns=[
        "pago_id","prestamo_id","fecha_pago","monto_pago","interes_aplicado","capital_aplicado",
        "dias","observaciones","creado_en","metodo",
        "saldo_antes","saldo_despues","dias_mora","mora_aplicada","tipo_pago","adelanto_extra"
    ])
    parametros = pd.DataFrame(columns=["clave","valor"])
    integrantes = pd.DataFrame(columns=["integrante_id","nombre","identificacion","cupos","creado_en","actualizado_en"])
    aportes_tarifas = pd.DataFrame(columns=["anio","valor_por_cupo"])
    aportes_pagos = pd.DataFrame(columns=[
        "aporte_id","integrante_id","periodo","fecha_pago","cupos_pagados","monto_pagado","observaciones","creado_en"
    ])
...
def load_data(path=EXCEL_PATH):
    ...
    prestamos = ensure_cols(prestamos, [
        "prestamo_id","display_id","cliente_id","monto","tasa_mensual","fecha_inicio","plan_inicio","plazo_meses",
        "frecuencia","saldo_capital","estado","ultimo_calculo","cuota_fija","creado_en","actualizado_en"
    ])
    pagos = ensure_cols(pagos, [
        "pago_id","prestamo_id","fecha_pago","monto_pago","interes_aplicado","capital_aplicado",
        "dias","observaciones","creado_en","metodo",
        "saldo_antes","saldo_despues","dias_mora","mora_aplicada","tipo_pago","adelanto_extra"
    ])
    ...
    for i, row in prestamos.iterrows():
        try:
            if (pd.isna(row.get("cuota_fija")) and not pd.isna(row.get("monto")) and
                not pd.isna(row.get("tasa_mensual")) and not pd.isna(row.get("plazo_meses"))):
...
def simulador_cuotas_fijas(P: float, i_mensual: float, n: int, fecha_inicio: date):
    ...
    for k in range(1, n + 1):
        ...
        if k == n:
            ...
            saldo_nuevo = 0
        filas.append({
            "# CUOTA": k, "MES": fecha_k, "SALDO INICIAL": saldo,
            "CUOTA": cuota_ajustada, "INTERES": interes_k, "CAPITAL": capital_k,
...
def crear_cliente_si_no_existe(clientes_df, nombre, identificacion, telefono="", email=""):
    ...
    now = datetime.now()
    nuevo = {
        "cliente_id": cid, "nombre": nombre, "identificacion": identificacion,
        "telefono": telefono, "email": email, "creado_en": now, "actualizado_en": now
    }
...
def crear_prestamo(prestamos_df, cliente_id, monto, tasa_mensual_decimal, fecha_inicio, plazo_meses):
    ...
    nuevo = {
        "prestamo_id": pid, "display_id": display_id, "cliente_id": cliente_id,
        "monto": int(round(float(monto))), "tasa_mensual": float(tasa_mensual_decimal),
        "fecha_inicio": fecha_inicio, "plan_inicio": fecha_inicio, "plazo_meses": int(plazo_meses), "frecuencia": "mensual",
        "saldo_capital": int(round(float(monto))), "estado": "activo", "ultimo_calculo": fecha_inicio,
        "cuota_fija": cuota_fija, "creado_en": now, "actualizado_en": now
    }
...
def registrar_pago_cuota(prestamos_df, pagos_df, prestamo_id, fecha_pago, adelanto_extra=0, observaciones=""):
    ...
    prestamos_df.loc[prestamos_df["prestamo_id"] == prestamo_id, ["saldo_capital","ultimo_calculo","actualizado_en"]] = [
        saldo_despues, fecha_pago, datetime.now()
    ]
    if saldo_despues <= 0:
        prestamos_df.loc[prestamos_df["prestamo_id"] == prestamo_id, "estado"] = "cerrado"

    pago_row = {
        "pago_id": str(uuid.uuid4()), "prestamo_id": prestamo_id, "fecha_pago": fecha_pago, "monto_pago": monto_total,
        "interes_aplicado": interes_mes, "capital_aplicado": capital_total, "dias": 0, "observaciones": observaciones,
        "creado_en": datetime.now(), "metodo": "cuota_mensual",
        "saldo_antes": saldo_antes, "saldo_despues": saldo_despues,
        "dias_mora": dias_mora, "mora_aplicada": mora_aplicada,
...
st.markdown(
    f"""
    <h2>💰 {APP_NAME}</h2>
    Contabilidad en Excel · Cuota fija · Mora por días · Adelanto extra · Re‑amortización · Aportes · Rendimiento anual
    """,
    unsafe_allow_html=True,
)
...
if uploaded_calc is not None and st.sidebar.button("➡️ Importar primera hoja", key="side_import_btn"):
    try:
        # Importador simple: busca campos clave básicos.
        ...
        if isinstance(i_m, str):
            ...
            try:
                ...
                tasa_mensual = 0.0
        clientes, cid = crear_cliente_si_no_existe(clientes, nombre_cliente or "Cliente Fondo Bonilla", identificacion=f"{(nombre_cliente or 'Cliente')[:20]}-AUTO")
        prestamos, pid = crear_prestamo(prestamos, cid, P, tasa_mensual, F0, n)
        save_data(clientes, prestamos, pagos, parametros)
        st.sidebar.success(f"Importado para '{nombre_cliente or 'Cliente Fondo Bonilla'}' (ID: {prestamos[prestamos['prestamo_id']==pid]['display_id'].iloc[0]}).")
...
# Tabs
TABS = ["📊 Panel", "➕ Nuevo Préstamo", "💳 Registrar Pago", "📑 Reportes", "🧮 Simulador", "🗃️ Datos / Parámetros", "♻️ Re‑amortización", "👥 Aportes"]
tabs = st.tabs(TABS)
...
with tabs[0]:
    ...
    def resumen_fondo_ext(prestamos_df, pagos_df, parametros_df, integ_df=None, aportes_pagos_df=None):
        capital_inicial = 0.0
        ...
        intereses_cobrados = float(pagos_df["interes_aplicado"].sum()) if not pagos_df.empty else 0.0
        capital_recuperado = float(pagos_df["capital_aplicado"].sum()) if not pagos_df.empty else 0.0
        ...
        total_por_cupo = (total_fondo / total_cupos) if total_cupos > 0 else 0.0
        hoy = date.today()
        if not pagos_df.empty and "fecha_pago" in pagos_df.columns:
            ...
            intereses_anio = float(pagos_df[mask_year]["interes_aplicado"].sum())
        else:
            intereses_anio = 0.0
        return {
    ...
    R = resumen_fondo_ext(prestamos, pagos, parametros, integrantes, aportes_pagos)
    ...
    c5.metric("Total por cupo", format_cop(R["total_por_cupo"]))
    ...
    if activos.empty:
        ...
        activos_show = activos[[
            "display_id","cliente","monto","tasa_%","plazo_meses","cuotas_restantes","saldo_capital",
            "estado","cuota_fija","plan_inicio","fecha_inicio","ultimo_calculo","creado_en"
        ]].copy()
...
with tabs[1]:
    ...
    with st.expander("Crear/Seleccionar cliente", expanded=False):
        with st.form("form_cliente"):
            ...
            with colA: nombre = st.text_input("Nombre", key="newloan_cli_nombre")
            with colB: identificacion = st.text_input("Identificación", key="newloan_cli_id")
            with colC: telefono = st.text_input("Teléfono", key="newloan_cli_tel")
            with colD: email = st.text_input("Email", key="newloan_cli_email")
    ...
    if clientes.empty:
        ...
        for _, row in clientes.iterrows():
            ...
            labels.append(lbl); id_map[lbl] = safe_str(row["cliente_id"])
        selected = st.selectbox("Cliente", options=labels, key="newloan_cli_select")
        cliente_id = id_map[selected]
        col1, col2, col3, col4 = st.columns(4)
        with col1: monto = st.number_input("Monto (COP)", min_value=0.0, value=1_000_000.0, step=50_000.0, key="newloan_monto")
        with col2: tasa_text = st.text_input("Tasa mensual (%)", value="3%", key="newloan_tasa")
        with col3: plazo_meses = st.number_input("Plazo (meses)", min_value=1, value=6, step=1, key="newloan_plazo")
        with col4: fecha_inicio = st.date_input("Fecha de inicio", value=date.today(), key="newloan_fecha")
                ...
                try:
                    ...
                    if saldo_despues > 0 and safe_int(adelanto_extra, default=0) > 0 and modo_reauto != "No re‑amortizar":
                        if modo_reauto == "Reducir plazo (mantener cuota)":
                            ...
                            prestamos.loc[prestamos["prestamo_id"] == pid, ["plazo_meses","cuota_fija","plan_inicio","actualizado_en"]] = [int(n_calc), int(cuota_calc), to_date(fecha_pago) or date.today(), datetime.now()]
                            ...
                            prestamos.loc[prestamos["prestamo_id"] == pid, ["plazo_meses","cuota_fija","plan_inicio","actualizado_en"]] = [int(n_rest), int(cuota_calc), to_date(fecha_pago) or date.today(), datetime.now()]
...
with tabs[3]:
    ...
    # Métrica de rendimiento anual (igual que v5)
    def total_fondo_fin_mes(prestamos_df, pagos_df, parametros_df, aportes_pagos_df, fecha_corte):
        ...
        aportes_hasta = (
            float(aportes_pagos_df[aportes_pagos_df["fecha_pago"].apply(lambda x: to_date(x) and to_date(x) <= fecha_corte)]["monto_pagado"].sum())
            if (aportes_pagos_df is not None and not aportes_pagos_df.empty) else 0.0
        )
...
with tabs[4]:
    ...
    colA, colB, colC, colD = st.columns(4)
    with colA: principal = st.number_input("Monto (P)", min_value=0.0, value=1_000_000.0, step=50_000.0, key="sim_monto")
    with colB: tasa_text = st.text_input("Tasa mensual (%)", value="3%", key="sim_tasa")
    with colC: n = st.number_input("Meses (n)", min_value=1, value=6, step=1, key="sim_n")
    with colD: f_inicio = st.date_input("Fecha 1ª cuota", value=date.today(), key="sim_fecha")
    if st.button("🧮 Calcular simulación", key="sim_calc"):
        try:
            ...
            st.session_state["sim"] = {"P": principal, "i_m": i_m, "n": int(n), "f_inicio": f_inicio,
        ...
        if modo == "Reducir cuota (mantener plazo)":
            ...
            if st.button("Aplicar re‑amortización", key="rea_apply_reduce_cuota"):
                try:
                    ...
                    prestamos.loc[prestamos["prestamo_id"] == pid_rea, ["tasa_mensual","plazo_meses","cuota_fija","plan_inicio","actualizado_en"]] = [
                        i_m, int(n_calc), int(cuota_calc), to_date(fecha_rea) or date.today(), datetime.now()
                    ]
                    save_data(clientes, prestamos, pagos, parametros)
            ...
            if st.button("Aplicar re‑amortización", key="rea_apply_reduce_plazo"):
                try:
                    ...
                    nueva_cuota_calc = calcular_cuota_fija(saldo_actual, i_m, plazo_in)
                    prestamos.loc[prestamos["prestamo_id"] == pid_rea, ["tasa_mensual","plazo_meses","cuota_fija","plan_inicio","actualizado_en"]] = [
                        i_m, int(plazo_in), int(nueva_cuota_calc), to_date(fecha_rea) or date.today(), datetime.now()
                    ]
                    save_data(clientes, prestamos, pagos, parametros)
...
with tabs[7]:
    ...
    for i, row in integrantes.iterrows():
        cupos_pag = st.number_input(
            ...
            min_value=0, max_value=safe_int(row["cupos"], default=0), value=0, step=1, key=f"ap_{row['integrante_id']}"
        )
        if cupos_pag > 0:
            monto = int(cupos_pag * nueva_tarifa)
            nuevos_aportes.append({
                "aporte_id": str(uuid.uuid4()), "integrante_id": row["integrante_id"], "periodo": periodo,
                "fecha_pago": fecha_pago_ap, "cupos_pagados": int(cupos_pag), "monto_pagado": monto,
                "observaciones": "", "creado_en": datetime.now()
            })
    ...
    def resumen_fondo_ext_simple():
        ...
        row = parametros[parametros["clave"] == "capital_inicial"]
        if not row.empty:
            try: capital_inicial = float(row.iloc[0]["valor"])
            except: pass
        ...
        intereses_cobrados = float(pagos["interes_aplicado"].sum()) if not pagos.empty else 0.0
        capital_recuperado = float(pagos["capital_aplicado"].sum()) if not pagos.empty else 0.0
        ...
        return (total_fondo / total_cupos) if total_cupos > 0 else 0.0
            ...
            if st.button("Aplicar igualación y agregar cupos", key="igual_btn_1"):
                if cupos_add > 0:
                    ...
                    if integ_mask.any():
                        ...
                        aporte_row = {
                            "aporte_id": str(uuid.uuid4()), "integrante_id": iid,
                            "periodo": f"{anio_actual}-{date.today().month:02d}",
                            "fecha_pago": fecha_igual, "cupos_pagados": int(cupos_add),
                            "monto_pagado": monto_eq, "observaciones": "igualación por cupos nuevos",
    ...
    with colI2:
        ...
        if submit_new:
            iid = str(uuid.uuid4()); now = datetime.now()
            row_new = {"integrante_id": iid, "nombre": nombre_new, "identificacion": ident_new,
                       "cupos": int(cupos_new), "creado_en": now, "actualizado_en": now}
            ...
            if int(cupos_new) > 0:
                aporte_row = {
                    "aporte_id": str(uuid.uuid4()), "integrante_id": iid,
                    "periodo": f"{anio_actual}-{date.today().month:02d}",
                    "fecha_pago": fecha_igual_new, "cupos_pagados": int(cupos_new), "monto_pagado": monto_eq,
                    "observaciones": "igualación por nuevo integrante", "creado_en": datetime.now()
                }
                aportes_pagos = pd.concat([aportes_pagos, pd.DataFrame([aporte_row])], ignore_index=True)
            save_aportes_data(integrantes, aportes_tarifas, aportes_pagos)
