...
def do_login(user: str, pwd: str) -> bool:
    ...
    for _, u in dict(udb).items():
        if str(u.get("username", "")).strip() == (user or "").strip():
            target = u
            break
    if not target:
        return False
    ...
    try:
        ok = stored and bcrypt.checkpw((pwd or "").encode("utf-8"), stored.encode("utf-8"))
    except Exception:
        ok = False
    if ok:
...
# Modal / diálogo de login con compatibilidad por versión (FIX de st.modal ausente)
if st.session_state.get("show_login_modal", False):
    # Usa st.modal si existe
    if hasattr(st, "modal"):
        with st.modal("Iniciar sesión", key="login_modal", max_width=600):
            ...
            with col_m2:
                if st.button("Cancelar", key="modal_login_cancel"):
                    st.session_state["show_login_modal"] = False
                    st.rerun()
    # Si no hay st.modal pero existe st.dialog
    ...
    # Fallback: login inline
        ...
        st.warning("Tu versión de Streamlit no soporta modal/dialog. Mostrando login inline.")
        u = st.text_input("Usuario", key="inline_user")
        p = st.text_input("Contraseña", type="password", key="inline_pwd")
        col_m1, col_m2 = st.columns([1,1])
        with col_m1:
            if st.button("Entrar", key="inline_login_confirm"):
                if do_login(u, p):
                    st.session_state["show_login_modal"] = False
                    st.rerun()
                else:
                    st.error("Usuario o contraseña incorrectos")
        with col_m2:
            if st.button("Cancelar", key="inline_login_cancel"):
                st.session_state["show_login_modal"] = False
                st.rerun()
...
def save_inversionista_data(inv_df, movs_df):
    ...
    # Forzar refresco para que el Panel lea valores actualizados
    _invalidate_cache()
...
if sel == TABS[0]:
    ...
    def resumen_fondo_ext(prestamos_df, pagos_df, parametros_df, integ_df=None, aportes_pagos_df=None, inv_df=None, inv_movs_df=None):
        ...
        # 2) Movimientos operativos del fondo
        ...
        # 3) Egresos al inversionista (dividendos + devolución de capital)  <<< PARCHE
        pagos_a_inversionista = 0.0
        if inv_movs_df is not None and not inv_movs_df.empty:
            egresos_mask = (
                inv_movs_df["tipo"].astype(str).str.strip().str.lower()
        ...
        # 4) Caja y total del fondo neteando pagos al inversionista
        ...
        # 5) Pasivo del inversionista (capital + ganancia pendiente)
        pasivo_inv = 0.0
        ...
        # 6) Intereses del año (sin cambios)
        hoy = date.today()
    ...
    # Funciones de reportes (sin cambio de inversión aquí, como acordamos)
    def total_fondo_fin_mes(prestamos_df, pagos_df, parametros_df, aportes_pagos_df, fecha_corte):
        ...
        # saldo_capital reconstruido con lo pagado a capital hasta la fecha
        saldo_capital = 0
