...
def render_igualacion_y_alta_rapida(prestamos, pagos, parametros, integrantes, aportes_tarifas, aportes_pagos):
    ...
    def _total_por_cupo_sin_inversion(prestamos_df, pagos_df, parametros_df, integ_df, aportes_pagos_df):
        capital_inicial = 0.0
        row = parametros_df[parametros_df["clave"] == "capital_inicial"] if not parametros_df.empty else parametros_df
        ...
        total_cupos = int(integ_df["cupos"].sum()) if not integ_df.empty else 0
    ...
    # --- Nuevo cupo (integrante existente)
    with colEQ1:
