...
# -*- coding: utf-8 -*-
# Fondo Bonilla (v8 + auth, NO SIDEBAR) — Login en modal/topbar y Logout en topbar
...
def do_login(user: str, pwd: str) -> bool:
    """Diagnóstico mínimo para saber por qué el login falla (sin cambiar tu flujo)."""
    udb = st.secrets.get("users", {})
    if not udb:
        st.error("No se encontraron usuarios en st.secrets['users'] (diccionario vacío).")
        return False

    # Buscar el usuario EXACTO (sensible a mayúsculas) tal como está en secrets
    target = None
    ...
    if not target:
        st.error(f"Usuario '{user_in}' no existe en st.secrets['users'].")
        return False

    phash = str(target.get("password_hash", "")).strip()
    if not phash:
        st.error(f"El usuario '{target.get('username')}' no tiene 'password_hash' configurado en secrets.")
        return False

    # Info de diagnóstico (segura): longitud y prefijo del hash
    try:
        st.caption(f"Diagnóstico: hash len={len(phash)} · prefijo={phash[:4]}")
    except Exception:
        pass

    try:
        ok = bcrypt.checkpw((pwd or "").encode("utf-8"), phash.encode("utf-8"))
    except Exception as e:
        st.error(f"Error validando bcrypt: {e}")
        return False

    if ok:
        st.session_state["auth_user"] = target.get("username", user_in)
        st.session_state["auth_role"] = target.get("role", "reader")
        return True

    st.error("Contraseña incorrecta para el usuario dado (bcrypt).")
    return False
...
with col_user:
    if "auth_user" in st.session_state:
        st.markdown(f"**Usuario:** {st.session_state['auth_user']} · **Rol:** {st.session_state['auth_role']}")
...
# Modal de login (OJO: si tu versión no tiene st.modal, esto dará AttributeError)
...
if sel == TABS[0]:
    ...
    def resumen_fondo_ext(prestamos_df, pagos_df, parametros_df, integ_df=None, aportes_pagos_df=None, inv_df=None, inv_movs_df=None):
        ...
        # --- Egresos inversionista (dividendos + cancelación) — bloque robusto único ---
