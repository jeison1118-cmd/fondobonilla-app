...
components.html(
    ...
    <link rel="apple-touch-icon" sizes="180x180" href="https://raw.githubusercontent.com/jeison1118-cmd/fondobonilla-app/main/assets/apple-touch-icon.png">
...
# ================================
# ELIMINACIONES CONTROLADAS (ADMIN)
# ================================

def eliminar_cliente(clientes_df, prestamos_df, cliente_id):
    prestamos_cliente = prestamos_df[
        (prestamos_df["cliente_id"] == cliente_id) &
        (prestamos_df["estado"].str.lower() != "cerrado")
    ]

    if not prestamos_cliente.empty:
        raise ValueError("El cliente tiene préstamos activos. No se puede eliminar.")

    clientes_df = clientes_df[clientes_df["cliente_id"] != cliente_id]
    return clientes_df


def eliminar_prestamo(prestamos_df, pagos_df, prestamo_id):
    prestamos_df = prestamos_df[prestamos_df["prestamo_id"] != prestamo_id]
    pagos_df = pagos_df[pagos_df["prestamo_id"] != prestamo_id]
    return prestamos_df, pagos_df


def eliminar_pago(pagos_df, prestamos_df, pago_id):
    pago = pagos_df[pagos_df["pago_id"] == pago_id]

    if pago.empty:
        raise ValueError("Pago no encontrado")

    prestamo_id = pago.iloc[0]["prestamo_id"]

    # eliminar pago
    pagos_df = pagos_df[pagos_df["pago_id"] != pago_id]

    # recalcular saldo del préstamo
    pagos_rest = pagos_df[pagos_df["prestamo_id"] == prestamo_id]
    capital_pagado = pagos_rest["capital_aplicado"].astype(float).sum()

    mask = prestamos_df["prestamo_id"] == prestamo_id
    monto = float(prestamos_df.loc[mask, "monto"].values[0])

    prestamos_df.loc[mask, "saldo_capital"] = monto - capital_pagado
...
def crear_integrante(integrantes_df: pd.DataFrame, nombre: str, identificacion: str, cupos_iniciales: int):
    ...
    nuevo = {
        ...
        "identificacion": safe_str(identificacion),
        "cupos": int(cupos_iniciales or 0),
        "creado_en": now,
        "actualizado_en": now,
    }
    integrantes_df = pd.concat([integrantes_df, pd.DataFrame([nuevo])], ignore_index=True)
    return integrantes_df, iid


def _valor_por_cupo_actual(prestamos_df, pagos_df, parametros_df, integrantes_df, aportes_pagos_df, inv_df=None, inv_movs_df=None) -> float:
    """Calcula total_por_cupo (sin inversionista) con la misma lógica del Panel."""
    capital_inicial = 0.0
    row = parametros_df[parametros_df.get("clave") == "capital_inicial"] if not parametros_df.empty else pd.DataFrame()
    if not row.empty:
        try:
            capital_inicial = float(row.iloc[0].get("valor", 0) or 0)
        except Exception:
            capital_inicial = 0.0
    ...
    aportes_cobrados = float(aportes_pagos_df.get("monto_pagado", pd.Series(dtype=float)).sum()) if (aportes_pagos_df is not None and not aportes_pagos_df.empty) else 0.0

    if inv_df is None or inv_movs_df is None:
        try:
            inv_df, inv_movs_df = load_inversionista()
        except Exception:
            inv_df, inv_movs_df = (pd.DataFrame(), pd.DataFrame())

    pagos_a_inversionista = 0.0
    if inv_movs_df is not None and not inv_movs_df.empty:
        tipos = inv_movs_df.get("tipo", pd.Series(dtype=object)).astype(str).str.strip().str.lower()
        mask = tipos.isin(["dividendo", "cancelacion"])
        try:
            pagos_a_inversionista = float(pd.to_numeric(inv_movs_df.loc[mask, "monto"], errors="coerce").fillna(0).sum())
        except Exception:
            pagos_a_inversionista = 0.0
    ...
    total_cupos = int(float(integrantes_df.get("cupos", pd.Series(dtype=float)).sum())) if (integrantes_df is not None and not integrantes_df.empty) else 0
    if total_cupos <= 0:
        return 0.0
    return float(total_sin_inversion) / float(total_cupos)


def registrar_ajuste_por_cupos(prestamos_df, pagos_df, parametros_df, integrantes_df, aportes_pagos_df,
                                integrante_id, delta_cupos, nota="", inv_df=None, inv_movs_df=None):
    """Registra igualación (delta>0) o devolución (delta<0) el día de hoy, usando el valor por cupo vigente."""
    dc = int(delta_cupos or 0)
    if dc == 0:
        return aportes_pagos_df, 0

    v_cupo = _valor_por_cupo_actual(prestamos_df, pagos_df, parametros_df, integrantes_df, aportes_pagos_df, inv_df, inv_movs_df)
    if v_cupo < 0:
        raise ValueError("El total por cupo actual es negativo; revisa los datos del fondo antes de continuar.")

    monto = int(round(v_cupo * dc))
    hoy = date.today()
    ap_row = {
        ...
        "cupos_pagados": 0,
        "monto_pagado": monto,
        "observaciones": (f"{'Igualación' if dc>0 else 'Devolución'} por {abs(dc)} cupo(s) @ {format_cop(v_cupo)} {nota}".strip()),
        "creado_en": datetime.now(),
    }
    aportes_pagos_df = pd.concat([aportes_pagos_df, pd.DataFrame([ap_row])], ignore_index=True)
    return aportes_pagos_df, monto
...
TABS = ["📊 Panel", "➕ Nuevo Préstamo", "💳 Registrar Pago", "📑 Reportes", "🧮 Simulador", "🗃️ Datos / Parámetros / Correciones", "♻️ Re‑amortización", "👥 Aportes", "👥 Integrantes", "👤 Inversionista"]
...
if sel == TABS[0]:
    ...
    if not can_edit():
        ...
        st.caption("Aquí puedes: crear integrante, agregar cupos, retirar cupos o retirar un integrante. "
                   "Cada ajuste genera automáticamente la igualación o devolución correspondiente (sin inversionista).")

        # Cargar inversionista una sola vez
        inv_df_tab, inv_movs_df_tab = load_inversionista()

        # 1) Crear integrante (+ igualación del mismo día)
        with st.expander("➕ Crear nuevo integrante", expanded=False):
            cA, cB, cC = st.columns(3)
            n_nombre = cA.text_input("Nombre", key="invz_new_nombre")
            n_ident  = cB.text_input("Identificación", key="invz_new_ident")
            n_cupos  = cC.number_input("Cupos iniciales", min_value=0, value=0, step=1, key="invz_new_cupos")
            try:
                v_preview = _valor_por_cupo_actual(prestamos, pagos, parametros, integrantes, aportes_pagos, inv_df_tab, inv_movs_df_tab)
                st.caption(f"Total por cupo (sin inversionista) actual: {format_cop(v_preview)} · "
                           f"Igualación estimada: {format_cop(int(round(v_preview * int(n_cupos or 0))))}")
            except Exception as e:
                st.warning(f"No se pudo calcular el total por cupo: {e}")
            if st.button("Crear integrante + registrar igualación", key="invz_btn_create"):
                try:
                    # 1) Crear con cupos=0 (temporal)
                    integ_tmp, iid = crear_integrante(integrantes, n_nombre, n_ident, 0)
                    # 2) Registrar igualación con el valor vigente (antes de subir cupos)
                    aportes_tmp, monto = registrar_ajuste_por_cupos(
                        prestamos, pagos, parametros, integrantes, aportes_pagos,
                        integrante_id=iid, delta_cupos=int(n_cupos or 0),
                        nota="(ingreso de integrante)", inv_df=inv_df_tab, inv_movs_df=inv_movs_df_tab
                    )
                    # 3) Actualizar cupos del nuevo integrante
                    integrantes = integ_tmp
                    mask_new = integrantes["integrante_id"] == iid
                    integrantes.loc[mask_new, ["cupos", "actualizado_en"]] = [int(n_cupos or 0), datetime.now()]
                    aportes_pagos = aportes_tmp
                    save_aportes_data(integrantes, aportes_tarifas, aportes_pagos)
                    st.success(f"Integrante creado. Igualación registrada por {format_cop(monto)}.")
                except Exception as e:
                    st.error(f"Error: {e}")

        # 2) Agregar cupos (+ igualación)
        with st.expander("➕ Agregar cupos a integrante", expanded=False):
            if integrantes.empty:
                st.info("No hay integrantes aún.")
            else:
                etiquetas, mapa = [], {}
                for _, r in integrantes.iterrows():
                    etiqueta = f"{safe_str(r['nombre'])} — cupos: {int(r.get('cupos',0) or 0)}"
                    etiquetas.append(etiqueta)
                    mapa[etiqueta] = (safe_str(r['integrante_id']), int(r.get('cupos',0) or 0))
                sel = st.selectbox("Integrante", options=etiquetas, key="invz_add_sel")
                iid_sel, cupos_act = mapa.get(sel, ("", 0))
                add_k = st.number_input("Cupos a agregar", min_value=0, value=0, step=1, key="invz_add_k")
                try:
                    v_preview = _valor_por_cupo_actual(prestamos, pagos, parametros, integrantes, aportes_pagos, inv_df_tab, inv_movs_df_tab)
                    st.caption(f"Total por cupo actual: {format_cop(v_preview)} · "
                               f"Igualación estimada: {format_cop(int(round(v_preview * int(add_k or 0))))}")
                except Exception as e:
                    st.warning(f"No se pudo calcular el total por cupo: {e}")
                if st.button("Agregar cupos + registrar igualación", key="invz_btn_add"):
                    try:
                        aportes_tmp, monto = registrar_ajuste_por_cupos(
                            prestamos, pagos, parametros, integrantes, aportes_pagos,
                            integrante_id=iid_sel, delta_cupos=int(add_k or 0),
                            nota="(ampliación de cupos)", inv_df=inv_df_tab, inv_movs_df=inv_movs_df_tab
                        )
                        nuevo_total = int(cupos_act + int(add_k or 0))
                        mask = integrantes["integrante_id"] == iid_sel
                        integrantes.loc[mask, ["cupos","actualizado_en"]] = [nuevo_total, datetime.now()]
                        aportes_pagos = aportes_tmp
                        save_aportes_data(integrantes, aportes_tarifas, aportes_pagos)
                        st.success(f"Cupos actualizados a {nuevo_total}. Igualación: {format_cop(monto)}.")
                    except Exception as e:
                        st.error(f"Error: {e}")

        # 3) Retirar cupos (– devolución)
        with st.expander("➖ Retirar cupos de un integrante", expanded=False):
            if integrantes.empty:
                st.info("No hay integrantes aún.")
            else:
                etiquetas, mapa = [], {}
                for _, r in integrantes.iterrows():
                    ...
                    etiquetas.append(etiqueta)
                    mapa[etiqueta] = (safe_str(r['integrante_id']), int(r.get('cupos',0) or 0))
                sel = st.selectbox("Integrante", options=etiquetas, key="invz_rem_sel")
                iid_sel, cupos_act = mapa.get(sel, ("", 0))
                max_k = int(cupos_act or 0)
                rem_k = st.number_input("Cupos a retirar", min_value=0, max_value=max_k, value=0, step=1, key="invz_rem_k")
                try:
                    v_preview = _valor_por_cupo_actual(prestamos, pagos, parametros, integrantes, aportes_pagos, inv_df_tab, inv_movs_df_tab)
                    st.caption(f"Total por cupo actual: {format_cop(v_preview)} · "
                               f"Devolución estimada: {format_cop(int(round(v_preview * int(rem_k or 0))) * -1)}")
                except Exception as e:
                    st.warning(f"No se pudo calcular el total por cupo: {e}")
                if st.button("Retirar cupos + registrar devolución", key="invz_btn_rem"):
                    try:
                        aportes_tmp, monto = registrar_ajuste_por_cupos(
                            prestamos, pagos, parametros, integrantes, aportes_pagos,
                            integrante_id=iid_sel, delta_cupos=-int(rem_k or 0),
                            nota="(retiro de cupos)", inv_df=inv_df_tab, inv_movs_df=inv_movs_df_tab
                        )
                        nuevo_total = int(cupos_act - int(rem_k or 0))
                        mask = integrantes["integrante_id"] == iid_sel
                        integrantes.loc[mask, ["cupos","actualizado_en"]] = [nuevo_total, datetime.now()]
                        aportes_pagos = aportes_tmp
                        save_aportes_data(integrantes, aportes_tarifas, aportes_pagos)
                        st.success(f"Cupos actualizados a {nuevo_total}. Devolución: {format_cop(monto)}.")
                    except Exception as e:
                        st.error(f"Error: {e}")

        # 4) Retirar integrante (cupos→0 + devolución total)
        with st.expander("🧹 Retirar integrante (dejar cupos en 0)", expanded=False):
            if integrantes.empty:
                st.info("No hay integrantes aún.")
            else:
                etiquetas, mapa = [], {}
                for _, r in integrantes.iterrows():
                    ...
                    etiquetas.append(etiqueta)
                    mapa[etiqueta] = (safe_str(r['integrante_id']), int(r.get('cupos',0) or 0))
                sel = st.selectbox("Integrante a retirar", options=etiquetas, key="invz_out_sel")
                iid_sel, cupos_act = mapa.get(sel, ("", 0))
                if st.button("Retirar integrante (cupos a 0) + registrar devolución", key="invz_btn_out"):
                    try:
                        if int(cupos_act or 0) > 0:
                            aportes_tmp, monto = registrar_ajuste_por_cupos(
                                prestamos, pagos, parametros, integrantes, aportes_pagos,
                                integrante_id=iid_sel, delta_cupos=-int(cupos_act or 0),
                                nota="(retiro de integrante)", inv_df=inv_df_tab, inv_movs_df=inv_movs_df_tab
                            )
                            aportes_pagos = aportes_tmp
                        else:
                            monto = 0
                        mask = integrantes["integrante_id"] == iid_sel
                        integrantes.loc[mask, ["cupos","actualizado_en"]] = [0, datetime.now()]
                        save_aportes_data(integrantes, aportes_tarifas, aportes_pagos)
                        st.success(f"Integrante retirado. Cupos=0. Devolución: {format_cop(monto)}.")
                        st.info("No se elimina la fila para preservar el histórico.")
                    except Exception as e:
                        st.error(f"Error: {e}")
