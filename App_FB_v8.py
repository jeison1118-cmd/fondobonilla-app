...
def save_data(clientes, prestamos, pagos, parametros, path=EXCEL_PATH):
    # Persistencia REAL en Google Sheets (no en Excel local)
    write_df("clientes", clientes)
    write_df("prestamos", prestamos)
    write_df("pagos", pagos)
    write_df("parametros", parametros)
    # Invalidar cache y refrescar data en UI
    _invalidate_cache()
    try:
        import streamlit as st
        st.session_state.pop("data_cache", None)
        ...
        pass
    # (Opcional) si quieres conservar el Excel local, descomenta:
