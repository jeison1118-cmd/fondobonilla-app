...
# -*- coding: utf-8 -*-
# Fondo Bonilla (v8 + auth, FIX) — Soluciona "primer clic" en login/logout y navegación
...
def logout():
    # Botón de cerrar sesión con rerun modern
...
def load_data():
    ...
    prestamos = ensure_cols(
        prestamos,
        ["prestamo_id","display_id","cliente_id","monto","tasa_mensual","fecha_inicio","plan_inicio","plazo_meses","frecuencia","saldo_capital","estado","ultimo_calculo","cuota_fija","creado_en","actualizado_en"],
    )
    pagos = ensure_cols(
        pagos,
        ["pago_id","prestamo_id","fecha_pago","monto_pago","interes_aplicado","capital_aplicado","dias","observaciones","creado_en","metodo","saldo_antes","saldo_despues","dias_mora","mora_aplicada","tipo_pago","adelanto_extra"],
    )
...
if can_edit():
    ...
    st.sidebar.button("💾 Guardar todo en Google Sheets", key="side_save_disabled", disabled=True)
...
if can_edit():
    ...
    st.sidebar.button("🗄️ Crear backup", key="side_backup_disabled", disabled=True)
...
if sel == TABS[0]:
    ...
    def resumen_fondo_ext(prestamos_df, pagos_df, parametros_df, integ_df=None, aportes_pagos_df=None, inv_df=None, inv_movs_df=None):
        ...
        hoy = date.today()
        if not pagos_df.empty and "fecha_pPago" in pagos_df.columns:
