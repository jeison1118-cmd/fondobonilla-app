...
inject_header()
# ensure_base_sheets() # desactivado: ya existen pestañas en el Sheet
# Manejo de señal de recarga (si se ha solicitado)
if st.session_state.get("__do_reload__"):
    st.session_state.pop("__do_reload__", None)
    d = read_all()
    st.session_state["data_cache"] = d
data = get_data()
...
st.sidebar.divider()
with st.sidebar.expander("🔧 Avanzado / Admin", expanded=False):
    if st.button("♻️ Recargar datos desde Google Sheets", key="side_reload"):
        safe_reload_data()
...
# Botón de backup/export desde Google Sheets (snapshot remoto)
if st.button("🗄️ Crear backup (exportar XLSX del Sheet)", key="report_backup_btn"):
    dst = create_backup()
    if dst:
        st.success(f"Backup creado: {dst}")
    else:
        st.warning("No se pudo crear el backup.")
# -------- Simulador --------
with tabs[4]:
