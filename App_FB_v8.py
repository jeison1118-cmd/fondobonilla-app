...
# === Igualación y alta rápida (sin inversión) ===
...
def _total_por_cupo_sin_inversion(prestamos_df, pagos_df, parametros_df, integrantes_df, aportes_pagos_df):
    # Caja: capital_inicial - desembolsos + (intereses + capital_recuperado + aportes)
    capital_inicial = 0.0
    row = parametros_df[parametros_df["clave"] == "capital_inicial"]
    if not row.empty:
        try:
    ...
    # Pasivo del inversionista (capital_inicial + ganancia_pendiente)
    inv_df, inv_movs_df = load_inversionista()
    pasivo_inv = 0.0
    if inv_df is not None and not inv_df.empty:
        try:
    ...
    total_sin_inversion = total_fondo - pasivo_inv
    total_cupos = int(integrantes_df["cupos"].sum()) if not integrantes_df.empty else 0
...
with colI1:
    ...
    if integrantes.empty:
        ...
        cupos_add = st.number_input("Cupos a agregar", min_value=1, value=1, step=1, key="igual_cupos")
...
with colI2:
    ...
    with st.form("form_nuevo_integrante"):
        ...
        if submit_new:
            ...
            # Si existe la identificación, acumular cupos (evitar duplicado)
            same_idx = None
