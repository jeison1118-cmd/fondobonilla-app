...
def crear_integrante(integrantes_df, nombre, identificacion, cupos_iniciales):
...
def _valor_por_cupo_actual(prestamos_df, pagos_df, parametros_df, integrantes_df, aportes_pagos_df) -> float:
    """Calcula total_por_cupo (sin inversionista) con la lógica del panel, sin depender de funciones locales."""
    # capital inicial
    capital_inicial = 0.0
    ...
    # egresos a inversionista: leer hojas existentes
    try:
    ...
    # total prestado (saldo capital total en libros)
    ...
    # pasivo inversionista (capital inicial + ganancia pendiente o anual)
    pasivo_inv = 0.0
...
def registrar_ajuste_por_cupos(prestamos_df, pagos_df, parametros_df, integrantes_df, aportes_pagos_df,
                                integrante_id, delta_cupos, nota=""):
    """
    Registra la igualación (delta>0) o devolución (delta<0) en aportes_pagos con fecha/período del día.
    Usa el total_por_cupo 'sin inversionista' vigente ANTES de actualizar cupos.
    """
    dc = int(delta_cupos or 0)
    if dc == 0:
        return aportes_pagos_df, 0

    v_cupo = _valor_por_cupo_actual(prestamos_df, pagos_df, parametros_df, integrantes_df, aportes_pagos_df)
    if v_cupo < 0:
        raise ValueError("El total por cupo actual es negativo; revisa los datos del fondo antes de continuar.")

    monto = int(round(v_cupo * dc))  # positivo = igualación; negativo = devolución
    hoy = date.today()
    ap_row = {
        ...
        "fecha_pago": hoy,
        "cupos_pagados": 0,  # no es el pago del mes; es ajuste extraordinario
        "monto_pagado": monto,
...
if sel == TABS[0]:
    ...
    if not can_edit():
        ...
        with st.expander("➕ Crear nuevo integrante", expanded=False):
            ...
            # Preview usando estado actual (sin aplicar todavía)
            try:
                v_preview = _valor_por_cupo_actual(prestamos, pagos, parametros, integrantes, aportes_pagos)
            ...
            if st.button("Crear integrante + registrar igualación", key="invz_btn_create"):
                try:
                    # Creamos el integrante y obtenemos su iid (aún no persistimos ajustes de caja)
                    integ_tmp, iid = crear_integrante(integrantes, n_nombre, n_ident, int(n_cupos or 0))

                    # Registramos la igualación ANTES de persistir (usa valor por cupo vigente)
                    aportes_tmp, monto = registrar_ajuste_por_cupos(
                        ...
                        nota="(ingreso de integrante)"
                    )

                    # Persistimos: integrantes (con cupos) + aportes_pagos (ajuste)
        ...
        # --- 2) Agregar cupos a un integrante existente (+ igualación)
        with st.expander("➕ Agregar cupos a integrante", expanded=False):
            if integrantes.empty:
                ...
                if st.button("Agregar cupos + registrar igualación", key="invz_btn_add"):
                    try:
                        # 1) registrar el ajuste ANTES de tocar cupos
                        aportes_tmp, monto = registrar_ajuste_por_cupos(
                            prestamos, pagos, parametros, integrantes, aportes_pagos,
                            integrante_id=iid_sel, delta_cupos=int(add_k or 0),
                            nota="(ampliación de cupos)"
                        )
                        # 2) actualizar cupos
        ...
        with st.expander("➖ Retirar cupos de un integrante", expanded=False):
            if integrantes.empty:
                ...
                if st.button("Retirar cupos + registrar devolución", key="invz_btn_rem"):
                    try:
                        # 1) registrar devolución (delta negativo) ANTES del cambio
                        aportes_tmp, monto = registrar_ajuste_por_cupos(
                            prestamos, pagos, parametros, integrantes, aportes_pagos,
                            integrante_id=iid_sel, delta_cupos=-int(rem_k or 0),
                            nota="(retiro de cupos)"
                        )
                        # 2) actualizar cupos
        ...
        # --- 4) Retirar integrante (cupos→0 + devolución total si aplica)
                ...
                if st.button("Retirar integrante (cupos a 0) + registrar devolución", key="invz_btn_out"):
                    try:
                        if int(cupos_act or 0) > 0:
                            # devolución por todos los cupos actuales
                            aportes_tmp, monto = registrar_ajuste_por_cupos(
                                prestamos, pagos, parametros, integrantes, aportes_pagos,
                                integrante_id=iid_sel, delta_cupos=-int(cupos_act or 0),
                                nota="(retiro de integrante)"
                            )
                            aportes_pagos = aportes_tmp
                        else:
                            monto = 0
