...
# ================== NUEVO: función de Igualación y alta rápida (v8, sin inversión) ==================
def render_igualacion_y_alta_rapida(prestamos, pagos, parametros, integrantes, aportes_tarifas, aportes_pagos):
    ...
    # --- 1) Calcular Total por cupo (sin inversión)
    def _total_por_cupo_sin_inversion(prestamos_df, pagos_df, parametros_df, integ_df, aportes_pagos_df):
        capital_inicial = 0.0
        row = parametros_df[parametros_df.get("clave") == "capital_inicial"] if isinstance(parametros_df, pd.DataFrame) else pd.DataFrame()
        if not row.empty:
            try:
                capital_inicial = float(row.iloc[0].get("valor", 0.0) or 0.0)
            except Exception:
                capital_inicial = 0.0

        total_desembolsado = float(prestamos_df.get("monto", pd.Series(dtype=float)).sum()) if isinstance(prestamos_df, pd.DataFrame) and not prestamos_df.empty else 0.0
        intereses_cobrados = float(pagos_df.get("interes_aplicado", pd.Series(dtype=float)).sum()) if isinstance(pagos_df, pd.DataFrame) and not pagos_df.empty else 0.0
        capital_recuperado = float(pagos_df.get("capital_aplicado", pd.Series(dtype=float)).sum()) if isinstance(pagos_df, pd.DataFrame) and not pagos_df.empty else 0.0
        aportes_cobrados = float(aportes_pagos_df.get("monto_pagado", pd.Series(dtype=float)).sum()) if isinstance(aportes_pagos_df, pd.DataFrame) and not aportes_pagos_df.empty else 0.0
        saldo_capital_total = float(prestamos_df.get("saldo_capital", pd.Series(dtype=float)).sum()) if isinstance(prestamos_df, pd.DataFrame) and not prestamos_df.empty else 0.0
        ...
        total_cupos = int(integ_df["cupos"].sum()) if isinstance(integ_df, pd.DataFrame) and not integ_df.empty else 0
    ...
    with colEQ1:
        ...
        if integrantes.empty:
            ...
            if st.button("Aplicar igualación y agregar cupos", key="eq_btn_exist"):
                try:
                    integ_mask = integrantes["nombre"].astype(str).str.strip().str.lower() == nombre_sel.strip().lower()
                    if not integ_mask.any():
                        ...
                        ap_new = pd.concat([aportes_pagos, pd.DataFrame([aporte_row])], ignore_index=True)
                        ...
                        save_aportes_data(integrantes, aportes_tarifas, ap_new)
                        st.success(f"Cupos agregados (+{int(cupos_add)}). Igualación registrada: {format_cop(monto_eq)}")
                        return integrantes, aportes_tarifas, ap_new
            ...
            if submit_new:
                try:
                    ...
                    if same_idx is not None:
                        iid = integrantes.loc[same_idx, "integrante_id"]
                        cupos_act = safe_int(integrantes.loc[same_idx, "cupos"], default=0)
                        integrantes.loc[same_idx, ["nombre", "cupos", "actualizado_en"]] = [
                            nombre_new or integrantes.loc[same_idx, "nombre"],
                            cupos_act + cupos_int,
                            datetime.now()
                        ]
                    else:
    ...
    return integrantes, aportes_tarifas, aportes_pagos
# ================== FIN FUNCIÓN NUEVA ==================
...
# --- Llamar a la sección nueva (igualación + alta rápida) ---
integrantes, aportes_tarifas, aportes_pagos = render_igualacion_y_alta_rapida(
    prestamos, pagos, parametros, integrantes, aportes_tarifas, aportes_pagos
)
