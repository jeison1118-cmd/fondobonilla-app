...
# -*- coding: utf-8 -*-
# Fondo Bonilla (v8 + auth) — Excel + UI con Inversionista (Teresa Pérez)
...
# --- Google Sheets auth + helpers (Service Account via st.secrets o service_account.json) ---
import requests
import gspread
...
# --- Helpers de robustez: backoff en lecturas y write-through cache ---
import time

def _read_ws_with_backoff(ws, evaluate_formulas=False, **options):
    """Lee un worksheet con reintentos exponenciales ante 429 (quota)."""
    delay = 0.5
...
def _write_and_update_cache(sheet_name: str, df: pd.DataFrame):
    """Escribe en Google Sheets y actualiza la copia local en session_state (write-through)."""
...
def _gs_credentials():
    ...
    if not json_path.exists():
        st.error("Faltan credenciales. Usa Secrets o coloca service_account.json junto al script.")
        st.stop()
...
def _get_ws(worksheet_name: str):
    """Obtiene la pestaña; la crea solo si NO existe."""
    sh = _get_spreadsheet()
    try:
...
# Exportar el Google Sheet completo a .xlsx (backup/descarga)

def export_sheet_to_xlsx(local_path: str = "/tmp/fondo.xlsx"):
    creds = _gs_credentials().with_scopes(["https://www.googleapis.com/auth/drive.readonly"])
...
# --- Cache de lecturas agrupadas (minimiza peticiones / evita 429) ---
...
# --- Persistencia Excel local opcional (no usada en GSheets, pero útil para descargas) ---

def save_data(clientes, prestamos, pagos, parametros, path=EXCEL_PATH):
    # Persistencia en Google Sheets + write-through cache local (sin recargar todo)
...
def create_backup():
    ...
    try:
        return export_sheet_to_xlsx(dst)
    except Exception:
        return None
...
# ensure_base_sheets() # desactivado: ya existen pestañas en el Sheet
...
if st.sidebar.button('♻️ Recargar datos desde Google Sheets', key='side_reload'):
    # Recargar puede permitirse a reader (no modifica el Sheet)
    _invalidate_cache()
    st.session_state.pop('data_cache', None)
    st.experimental_rerun()

# Guardar solo si admin (mostrar botón deshabilitado para reader)
st.sidebar.button(
    "💾 Guardar todo en Google Sheets",
    key="side_save",
    disabled=not can_edit(),
)
if can_edit() and st.session_state.get("side_save"):
    save_data(clientes, prestamos, pagos, parametros)
    save_aportes_data(integrantes, aportes_tarifas, aportes_pagos)
...
# Backup solo admin
st.sidebar.button("🗄️ Crear backup", key="side_backup", disabled=not can_edit())
if can_edit() and st.session_state.get("side_backup"):
    dst = create_backup()
...
if can_edit():
    ...
    st.sidebar.caption("📥 Importación disponible solo para admin")
...
# Fallback de navegación con st.radio (compatible con versiones anteriores de Streamlit)
_default_tab = st.session_state.get("active_tab", TABS[0])
try:
    _idx = TABS.index(_default_tab)
except Exception:
    _idx = 0
sel = st.radio("Navegación", options=TABS, index=_idx, key="nav_tabs")
st.session_state["active_tab"] = sel

# --- Render condicional por pestaña ---
# -------- Panel --------
if sel == TABS[0]:
    ...
    if activos.empty:
        ...
        activos_show = normalize_datetime_cols(
            activos_show, ["plan_inicio", "fecha_inicio", "ultimo_calculo", "creado_en"], to_string=True
        )
        st.dataframe(activos_show, use_container_width=True)

# -------- Nuevo Préstamo (ADMIN) --------
elif sel == TABS[1]:
...
# -------- Reportes (READER OK) --------
    ...
    # Descarga Excel: disponible en reader; si quieres restringir, pon disabled=not can_edit()
                ...
                if st.button("Registrar dividendos (70.000)", key="inv_btn_div"):
                    try:
                        ...
                        if pend <= 0:
                            ...
                            save_inversionista_data(inv_df, inv_movs_df)
                            data = get_data()
                            inv_df = data["inversionista"]
                            inv_movs_df = data["inversionista_movs"]
