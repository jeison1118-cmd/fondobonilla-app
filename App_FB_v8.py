...
# Modal de login (compatibilidad por versión de Streamlit: NO cambia tu do_login)
if st.session_state.get("show_login_modal", False):
    if hasattr(st, "modal"):
        # Flujo original con st.modal
        ...
        # Fallback visual si tu versión no tiene st.modal (NO cambia la validación)
...
if sel == TABS[0]:
    ...
    def resumen_fondo_ext(prestamos_df, pagos_df, parametros_df, integ_df=None, aportes_pagos_df=None, inv_df=None, inv_movs_df=None):
        ...
        # --- Egresos al inversionista (dividendos + cancelación), robusto a texto/espacios/mayúsculas ---
        ...
        # Caja neta (restando pagos al inversionista)
